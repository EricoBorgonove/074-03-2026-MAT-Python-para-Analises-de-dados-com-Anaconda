*******

# 📌 Sumário

1. [Introdução](#introdução)
2. [Módulos do Curso](#módulos-do-curso)
3. [Exercícios](#exercícios)
4. [Links Úteis](#links-úteis)

*******

# 📖 Introdução

> Seja muito bem-vindo(a) à **Turma de Março de 2026** do **Curso de Python para Análise de dados com Anaconda** oferecido pelo [Instituto Flexpeak](http://ifpk.com.br)! Estamos muito felizes em tê-lo(a) conosco e esperamos que você tenha uma experiência de aprendizado enriquecedora e gratificante. Caso precise de qualquer suporte ou tenha alguma dúvida, não hesite em entrar em contato conosco. Estamos aqui para ajudá-lo(a) em sua jornada de aprendizado.

Nosso objetivo é que, ao final da jornada, você seja capaz de:

- 🐍 Utilizar **Python** como ferramenta para análise de dados  
- 📊 Trabalhar com **datasets reais**  
- 🧠 Aplicar **técnicas básicas de análise exploratória de dados**  
- ⚙️ Utilizar o **Anaconda** para gerenciar ambientes e bibliotecas  
- 📈 Produzir **visualizações e insights a partir de dados**

---

# 🎯 Objetivos do Curso

Durante o curso você irá aprender a:

- Compreender o funcionamento do **ecossistema científico do Python**
- Instalar e configurar corretamente o **Anaconda**
- Criar e gerenciar **ambientes virtuais**
- Utilizar ferramentas como:

  - **Jupyter Notebook**
  - **Pandas**
  - **NumPy**
  - **Matplotlib**
  - **Seaborn**

- Realizar:

  - limpeza de dados
  - transformação de dados
  - análise exploratória
  - geração de gráficos
  - interpretação de resultados

---

# 🧰 Ferramentas Utilizadas

Durante o curso utilizaremos principalmente as seguintes ferramentas:

| Ferramenta | Função |
|-------------|--------|
| **Python** | Linguagem de programação utilizada na análise de dados |
| **Anaconda** | Distribuição que facilita a instalação de bibliotecas científicas |
| **Jupyter Notebook** | Ambiente interativo para análise e experimentação |
| **Pandas** | Manipulação e análise de dados |
| **NumPy** | Computação numérica |
| **Matplotlib / Seaborn** | Visualização de dados |

---

# 🧑‍💻 Metodologia

O curso foi desenvolvido com foco em **aprendizado prático**.

Você aprenderá através de:

- 📚 explicações conceituais
- 💻 exercícios práticos
- 📊 análise de datasets reais
- 🧪 experimentação em notebooks
- 🚀 pequenos projetos aplicados

Acreditamos que **programação se aprende programando**, portanto prepare-se para colocar a mão no código desde as primeiras aulas.

# 💡 Recomendações para os Alunos

Para aproveitar melhor o curso:

- 🧠 pratique os exercícios
- 🔁 repita os exemplos apresentados
- 📖 explore a documentação das bibliotecas
- 🧪 teste variações nos códigos
- ❓ traga dúvidas

O aprendizado em programação ocorre principalmente através da **experimentação**.

---

# 📬 Suporte

Caso precise de ajuda ou tenha alguma dúvida durante o curso, entre em contato conosco.
Estamos aqui para **apoiar sua jornada de aprendizado**.

---

# 🚀 Vamos começar!

Prepare seu ambiente, abra seu editor ou notebook e **vamos mergulhar no universo da análise de dados com Python!**

*******

> Este repositório utiliza a linguagem de marcação **Markdown** para formatar seus textos. O Markdown é amplamente utilizado na formatação de arquivos README, na criação de mensagens em fóruns de discussão online e na elaboração de textos mais ricos usando um editor de texto simples. Se você não está familiarizado com o Markdown, não se preocupe - é fácil de aprender! Você pode obter mais informações sobre como utilizá-lo através deste tutorial em [Português](https://github.com/luong-komorebi/Markdown-Tutorial/blob/master/README_pt-BR.md).

---


## 📂 Módulos do Curso

Aqui você encontrará os principais tópicos abordados no curso, organizados em arquivos específicos:

* 📥 [Entrada e Saída de Dados](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/entrada_saida.md)
* 🔢 [Operadores](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/operadores.md)
* 🔀 [Estruturas Condicionais](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/condicionais.md)
* 🔄 [Estruturas de Repetição](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/repeticao.md)
* 🛠 [Funções](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/funcoes.md)

---

## 📝 Exercícios

Para reforçar o aprendizado, pratique com os exercícios disponíveis nos seguintes tópicos:

* 🚀 [Iniciais](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/iniciais.md)
* ✏️ [Básicos](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/exercicio_python_basico.md)
* ❓ [Condicionais](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/Exerc%C3%ADcios%20condicionais.md)
* 🔁 [Repetição](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/EXERCICIOS-REPETICAO-logica.md)
* 🔄🔀 [Repetição e Condicionais](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/rep_cond.md)
* ⚙️ [Funções](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/funcoes_1.md)
* 🎲 [Diversos](https://github.com/EricoBorgonove/063-03-2025-TAR-PPI/blob/main/files/misturados.md)

---

## 🔗 Links Úteis

Aqui estão algumas ferramentas que podem auxiliar no desenvolvimento e prática de programação em Python:

### 🖥 IDEs e Ferramentas

1. [Visual Studio Code](https://code.visualstudio.com/) - Um dos editores de código mais utilizados.
2. [Git - Downloads](https://git-scm.com/downloads) - Controle de versão e colaboração.
3. [Github](https://www.github.com) - Hospedagem de código e colaboração.
4. [Python](https://www.python.org/) - Site oficial do Python.
5. [Python Online](https://www.online-python.com/) - Teste seus códigos diretamente no navegador.
6. [PythonOnline](https://pythononline.net/) - Outra opção para rodar Python online.

💡 _Aproveite o curso e bons estudos! 🚀_
